...
async fn main() -> Result<()> {
    ...
    let refresh_token_repository =
        Arc::new(PostgresRefreshTokenRepository::new(db_pool.clone()));
    ...
    let smtp_from_name = std::env::var("SMTP_FROM_NAME").unwrap_or_else(|_| "Financial Tracker".to_string());
    #[allow(clippy::expect_used)]
    ...
    let email_service =
        EmailService::new(smtp_host, smtp_port, smtp_user, smtp_pass, smtp_from_email, smtp_from_name)?;
    ...
    let delete_user = Arc::new(DeleteUser::new(user_repository.clone()));
    ...
    let deactivate_account = Arc::new(DeactivateAccount::new(account_repository.clone()));
    ...
    let delete_transaction = Arc::new(DeleteTransaction::new(transaction_repository.clone()));
    ...
    let deactivate_budget = Arc::new(DeactivateBudget::new(budget_repository.clone()));
